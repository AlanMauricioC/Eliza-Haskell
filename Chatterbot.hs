...
chatterbot botName botRules = do
  ...
  where
    botloop = do
      juan <-readMyStuff "ElizaRules.txt"
      let brain = rulesCompile juan
      putStr "\n: "
      question <- getLine
      answer <- stateOfMind brain
      putStrLn (botName ++ ": " ++ (present . answer . prepare) question)
      if  question == "a darle atomos" then do
        entrena botRules
        botloop else 
...
entrena:: [Oracion] ->IO ()
entrena x = do
  putStr "Ingresa la nueva pregunta\n:"
  question<-getLine
  answers<-answerList []
  let tmp=[(question,answers)] ++ x
  writeMyStuff "ElizaRules.txt" tmp
  --print(tmp)
  return ()
...
answerList:: [String]->IO [String]
answerList x=do
  putStr "Inserta la nueva respuesta\n:"
  answer<-getLine  
  let newList=x
  if (not . endOfDialog) answer then do
    let newList=x++ [answer]
    answerList newList 
    else return newList
-------------------------------------------------------
type Oracion = (String, [String])

{-Escribe los datos en un archivo-}
writeMyStuff :: FilePath -> [Oracion] -> IO ()
writeMyStuff filename somedata = writeFile filename (show somedata)
{-Lee los datos en un archivo-}
readMyStuff :: FilePath ->IO  [Oracion]
